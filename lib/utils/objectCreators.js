...
const getSuiteEndInfo = (suite, testFileName) => {
  let failed = false;
  const skippedTests = [];

  if (suite.tests != null) {
    failed = suite.tests.some((test) => test.state === FAILED);
    suite.tests.forEach((test) => {
      if (test.state === SKIPPED && test.id) {
        skippedTests.push({
          id: test.id,
          title: test.title,
          status: SKIPPED,
          parentId: suite.id,
          codeRef: testFileName ? getCodeRef(test.titlePath(), testFileName) : undefined,
        });
      }
    });
  }
