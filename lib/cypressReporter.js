...
class CypressReporter extends Mocha.reporters.Base {
  ...
  constructor(runner, config) {
    ...
    if (this.isFirstRun() || !config.reporterOptions.autoMerge) {
      ...
      const launchLogListener = (log) => this.worker.send({ event: reporterEvents.LAUNCH_LOG, log });
      const attributesListener = ({ attributes }) => this.worker.send({ event: reporterEvents.ADD_ATTRIBUTES, attributes });
      const descriptionListener = ({ description }) => this.worker.send({ event: reporterEvents.SET_DESCRIPTION, description });
      const testCaseId = (testCaseIdInfo) => this.worker.send({ event: reporterEvents.SET_TEST_CASE_ID, testCaseIdInfo });
      const screenshotListener = (screenshotInfo) => this.worker.send({ event: reporterEvents.CUSTOM_SCREENSHOT, screenshotInfo });
      const setStatusListener = (statusInfo) => this.worker.send({ event: reporterEvents.SET_STATUS, statusInfo });
      const setLaunchStatusListener = (statusInfo) => this.worker.send({ event: reporterEvents.SET_LAUNCH_STATUS, statusInfo });
