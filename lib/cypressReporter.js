...
class CypressReporter extends Mocha.reporters.Base {
  constructor(runner, initialConfig) {
    ...
    if (
      ...
      const screenshotListener = (details) =>
        this.worker.send({ event: reporterEvents.SCREENSHOT, details });
      ...
      const stepStartListener = (step) =>
        this.worker.send({ event: reporterEvents.STEP_START, step });
      const stepEndListener = (step) => this.worker.send({ event: reporterEvents.STEP_END, step });
      ...
      startIPCServer(
        (server) => {
          ...
          server.on(IPC_EVENTS.STEP_START, stepStartListener);
          server.on(IPC_EVENTS.STEP_END, stepEndListener);
        },
        (server) => {
          ...
          server.off(IPC_EVENTS.STEP_START, '*');
          server.off(IPC_EVENTS.STEP_END, '*');
        },
      );
