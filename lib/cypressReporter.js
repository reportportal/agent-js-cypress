...
class CypressReporter extends Mocha.reporters.Base {
  constructor(runner, initialConfig) {
    ...
    if (
      ...
      const setLaunchDescriptionListener = (description) =>
        this.worker.send({ event: reporterEvents.SET_LAUNCH_DESCRIPTION, description });
      ...
      startIPCServer(
        (server) => {
          ...
          server.on(IPC_EVENTS.SET_LAUNCH_DESCRIPTION, setLaunchDescriptionListener);
        },
        (server) => {
          ...
          server.off(IPC_EVENTS.SET_LAUNCH_DESCRIPTION, '*');
        },
      );
