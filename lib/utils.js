...
const getConfig = (initialConfig) => {
  ...
  if (
    ...
    attributes.push({ key: 'CI_BUILD_ID', value: process.env.CI_BUILD_ID });
  }

  if (process.env.BRANCH_NAME) {
    attributes.push({ key: 'BRANCH_NAME', value: process.env.BRANCH_NAME });
  }  

  if (process.env.PR_TITLE) {
    attributes.push({ key: 'PR_TITLE', value: process.env.PR_TITLE });
  }
