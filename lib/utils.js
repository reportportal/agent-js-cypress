...
const waitForFile = (globFilePath, interval = 500, timeout = 10000) =>
  new Promise((resolve, reject) => {
    let totalTime = 0;

    async function checkFileExistence() {
      // TODO: maybe replace with method from fs.promises
      ...
      if (files.length) {
        ...
        reject(
          new Error(
            `Timeout of ${timeout}ms reached, file ${globFilePath} does not exist. Try increasing the timeout.`,
          ),
        );
      } else {
        totalTime += interval;
...
const getVideoFile = async (specFileName, videosFolder = '**') => {
  if (!specFileName) {
    return null;
  }
  ...
  try {
    videoFilePath = await waitForFile(globFilePath);
  } catch (e) {
...
const prepareReporterOptions = (config) => {
  // copy this options from Cypress config to report portal options
