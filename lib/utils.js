...
const getFailedVideo = (filePath) => {
  const pattern = `**/*{j,t}s.mp4`;
  const files = glob.sync(pattern);
  const fileName = filePath.replace(/[a-z]+\//g,'')
  return files.length
    ? {
        name: `${fileName}.mp4`,
        type: 'video/mp4',
...
const getTestInfo = (test, testFileName, status, err) => ({
  ...
  err: (err && err.message) || err || test.err || (test.err && test.err.message) ? (test.err.message + test.err.stack + "\n" + test.body) : "",
  testFileName,
});
