...
const getFailedScreenshots = (testTitle) => {
  const patternFirstFailedScreenshot = `**/*${testTitle.replace(/[",',:]/g, '')} (failed).png`;
  const patternNumeratedFailedScreenshot = `**/*${testTitle.replace(/[",',:]/g, '')} (failed) (attempt *.png`;
  const firstFailedScreenshot = glob.sync(patternFirstFailedScreenshot) || [];
  const numeratedFailedScreenshots = glob.sync(patternNumeratedFailedScreenshot) || [];
  const files = firstFailedScreenshot.concat(numeratedFailedScreenshots);
  return (files || []).map((file, index) => ({
    name: `${testTitle}-${index + 1}`,
    type: 'image/png',
    content: base64Encode(file),
