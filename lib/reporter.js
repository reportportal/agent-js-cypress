...
class Reporter {
  constructor(config) {
    ...
    const reporterOptions = {
      ...
      skippedIsNotIssue: String(config.reporterOptions.skippedIssue).toLowerCase() === 'false',
    };
    this.client = new RPClient(reporterOptions, agentInfo);
  ...
  reportSkippedTests(suite) {
    const { skippedTests = [] } = suite;

    skippedTests.forEach((skippedTest) => {
      if (this.testItemIds.has(skippedTest.id)) {
        return;
      }

      const parentId = this.testItemIds.get(skippedTest.parentId);
      const startTestObj = getTestStartObject(skippedTest);
      ...
      promiseErrorHandler(startPromise, 'Fail to start skipped test');
      ...
      const finishPromise = this.client.finishTestItem(tempId, finishTestObj).promise;
      promiseErrorHandler(finishPromise, 'Fail to finish skipped test');
    });
  }
