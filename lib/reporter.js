...
class Reporter {
  ...
  stepStart(data) {
    ...
    this.steps.set(testStepId, { tempId, tempParentId: parent.tempId, testStepId });
    parent.steps.add(testStepId);
  }
  ...
  stepEnd(data) {
    ...
    const step = this.steps.get(testStepId);
    ...
    if (this.currentTestTempInfo) {
      this.currentTestTempInfo.steps.delete(testStepId);
    }
  }
  ...
  getCurrentStep() {
    if (this.currentTestTempInfo && this.currentTestTempInfo.steps.size > 0) {
      const testStepId = Array.from(this.currentTestTempInfo.steps.values())[
        this.currentTestTempInfo.steps.size - 1
      ];

      return this.steps.get(testStepId);
    }
