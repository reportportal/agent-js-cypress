...
class Reporter {
  constructor(config) {
    ...
    this.hooks = new Map();
    this.config = config;
    this.debug = config.reporterOptions.debug;
  ...
  runEnd() {
    const finishLaunchPromise = this.client
      ...
      .then(() => {
        ...
        return this.mergeParallelLaunches();
      });
  ...
  async mergeParallelLaunches() {
    ...
    try {
      ...
      const response = await this.client.restClient.retrieveSyncAPI(launchSearchUrl, {
        headers: this.client.headers,
      });
      this.logDebug(`Search launches response: ${response.content}`);
      ...
      this.logDebug(`Found launches: ${launchesInProgress.length}`);
      ...
      const request = this.client.getMergeLaunchesRequest(launchIds);
      request.description = this.config.reporterOptions.description;
      request.extendSuitesDescription = false;
      const mergeURL = 'launch/merge';
      await this.client.restClient.create(mergeURL, request, { headers: this.client.headers });
