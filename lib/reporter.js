...
class Reporter {
  ...
  suiteEnd(suite) {
    const isVideoRecordingEnabled = true; // TODO: use config option instead
    const reportVideos = true; // TODO: use config option instead
    const isRootSuite =
    ...
    if (isVideoRecordingEnabled && reportVideos && isRootSuite) {
  ...
  finishSuiteWithVideo(suiteInfo, suiteFinishObj) {
    const videoUploadOnPasses = this.config.reporterOptions.videoUploadOnPasses || false; // TODO: update parameter
    const failed = suiteFinishObj.status === testItemStatuses.FAILED;

    // do not upload video if root suite passes and videoUploadOnPasses is false
    if ((!failed && !videoUploadOnPasses) || !suiteInfo.testFileName) {
      this.finishSuite(suiteFinishObj, suiteInfo.tempId);
    } else {
      const sendVideoPromise = this.sendVideo(suiteInfo).then(() => {
        this.finishSuite(suiteFinishObj, suiteInfo.tempId);
      });
  ...
  async sendVideo(suiteInfo) {
    ...
    const videosFolder = this.config.reporterOptions.videosFolder; // TODO: update parameter
    const file = await getVideoFile(testFileName, videosFolder);
    if (!file) {
      return null;
    }
